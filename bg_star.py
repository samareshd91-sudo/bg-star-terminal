...
selected_tf = "5m" # FIXED: Changed from 1m to 5m to reduce market noise
...
# ================= ⚡ 4. CORE ENGINE (DATA & REFINED LOGIC) =================
def fetch_and_analyze(coin):
    try:
        # FIXED: Increased limit to 500 for accurate EMA 50 and RSI calculations
        ...
        # 4.2.1 FIXED REAL-TIME REVERSAL LOGIC (No Look-ahead bias)
        # Checking pivot points with a 2-candle confirmation delay (standard fractal logic)
        ...
        tol = 0.003 # 0.3% tolerance for price level matching
        ...
        if is_double_bottom: p_name, p_color, p_desc = "ডাবল বটম ✌️", "#00BFFF", "চার্টে ডাবল বটম (W) প্যাটার্ন তৈরি হয়েছে। বুলিশ রিভার্সাল।"
        elif is_inv_head_shoulders: p_name, p_color, p_desc = "ইনভার্স হেড এন্ড শোল্ডার 👤", "#00BFFF", "চার্টে ইনভার্স হেড এন্ড শোল্ডার তৈরি হয়েছে। স্ট্রং বুলিশ।"
        elif is_double_top: p_name, p_color, p_desc = "ডাবল টপ ⛰️", "#FF00FF", "চার্টে ডাবল টপ (M) প্যাটার্ন তৈরি হয়েছে। বিয়ারিশ রিভার্সাল।"
        elif is_head_shoulders: p_name, p_color, p_desc = "হেড এন্ড শোল্ডার 👤", "#FF00FF", "চার্টে হেড এন্ড শোল্ডার তৈরি হয়েছে। স্ট্রং বিয়ারিশ।"
        elif is_morning_star: p_name, p_color, p_desc = "মর্নিং স্টার 🌅", "#00FF00", "৩ ক্যান্ডেল মিলে স্ট্রং বুলিশ রিভার্সাল।"
        elif is_evening_star: p_name, p_color, p_desc = "ইভনিং স্টার 🌃", "#FF1744", "৩ ক্যান্ডেল মিলে স্ট্রং বিয়ারিশ রিভার্সাল।"
        elif is_bullish_engulfing: p_name, p_color, p_desc = "বুলিশ এনগালফিং 📈", "#00FF00", "বায়াররা সেলারদের গিলে ফেলেছে।"
        elif is_bearish_engulfing: p_name, p_color, p_desc = "বিয়ারিশ এনগালফিং 📉", "#FF1744", "সেলাররা বায়ারদের গিলে ফেলেছে।"
        ...
        # FIXED: Increased volume spike threshold to 3.0 to filter out 5m noise
        ...
        # 4.6 Strict Signal Logic
        signal_type = "NORMAL"
        
        if artistic_buy_pattern:
            signal_type = "ARTISTIC BUY"
        elif artistic_sell_pattern:
            signal_type = "ARTISTIC SELL"
...
for coin, data in all_data.items():
    if data['signal'] in ["STRONG BUY", "STRONG SELL", "ARTISTIC BUY", "ARTISTIC SELL"]:
        active_signals += 1
        
        is_buy = data['signal'] in ["STRONG BUY", "ARTISTIC BUY"]
        
        if data['signal'] == "ARTISTIC BUY":
            card_class = "global-alert-artistic-buy"
            color_main = "#00BFFF"
            icon = "🎨 আর্টিস্টিক বাই করার টাইম"
        elif data['signal'] == "ARTISTIC SELL":
            card_class = "global-alert-artistic-sell"
            color_main = "#FF00FF"
            icon = "🎨 আর্টিস্টিক সেল করার টাইম"
...
if active_signals == 0:
    st.markdown("""
        <div class="global-alert-normal">
            ⏳ বর্তমানে কোনো কয়েনে কনফার্ম সিগন্যাল নেই। <b>অপেক্ষা করুন...</b>
        </div>
    """, unsafe_allow_html=True)
...
if st.session_state.active_coin in all_data:
    ...
    with c3:
        if data['buyer_vol_spike']:
            v_val, v_color, v_desc = "বড় বায়ার অ্যাক্টিভ 🟢", "#00FF00", "অস্বাভাবিক ভলিউমে বায়িং হয়েছে।"
        elif data['seller_vol_spike']:
            v_val, v_color, v_desc = "বড় সেলার অ্যাক্টিভ 🔴", "#FF1744", "অস্বাভাবিক ভলিউমে সেলিং হয়েছে।"
        else:
            v_val, v_color, v_desc = "নরমাল ভলিউম ❄️", "#848E9C", "স্বাভাবিক ট্রেডিং চলছে।"
    ...
    # 6.3 Interactive Chart (Showing last 100 candles for better view)
...
# ================= 🔄 7. AUTO REFRESH LOOP (FIXED) =================
# FIXED: Increased wait time to 10 seconds for stable API calls and proper page reloading without blocking
time.sleep(10)
st.rerun()
