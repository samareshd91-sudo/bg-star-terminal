...
st.set_page_config(page_title="BG STAR: LEARN & EARN", layout="wide", initial_sidebar_state="collapsed")
...
st.markdown("""
    ...
    .logic-box { background: #1E2329; border-left: 3px solid #00BFFF; padding: 8px 12px; border-radius: 4px; margin-bottom: 6px; font-size: 12px; color: #EAECEF; }
    ...
    div.stButton > button { border-radius: 8px !important; font-weight: bold !important; width: 100% !important; background-color: #181A20 !important; color: #EAECEF !important; border: 1px solid #2B3139 !important; transition: 0.2s;}
    div.stButton > button:hover { border-color: #FCD535 !important; transform: scale(1.02);}
    </style>
...
if 'app_start_time' not in st.session_state: 
    ...
    if saved_data and isinstance(saved_data.get('bot_positions'), dict):
        st.session_state.update(saved_data)
        if st.session_state.active_coin not in SCALPING_COINS: st.session_state.active_coin = "BTC/USDT"
    else:
...
# ================= 🧠 GLOBAL AI RADAR (BAG 1 FIXED) =================
# একবার নিউজ ফেচ করবে, আর সব কয়েনের জন্য কাজ করবে (RAM বাঁচবে)
@st.cache_data(ttl=300) 
def get_global_ai_sentiment():
    try:
        resp = requests.get('https://cointelegraph.com/rss', timeout=5)
        root = ET.fromstring(resp.content)
        global_news = " ".join([item.text for item in root.findall('./channel/item/title')[:5]]) # Top 5 news
        
        response = requests.post("https://api-inference.huggingface.co/models/ProsusAI/finbert", headers={"Authorization": f"Bearer {HF_API_KEY}"}, json={"inputs": global_news}, timeout=5).json()
        if isinstance(response, list) and len(response) > 0 and isinstance(response[0], list): 
            return {"label": response[0][0]['label'], "news": global_news}
...
global_ai_data = get_global_ai_sentiment()
is_critical_danger = global_ai_data['label'] == "negative"
...
if is_critical_danger:
    st.markdown(f'<div style="background-color: #FF1744; color: white; padding: 10px; text-align: center; font-weight: bold; border-radius: 8px; margin-bottom: 15px;">🚨 AI WARNING: GLOBAL MARKET DANGER! 🚨<br><span style="font-size:11px; font-weight:normal;">{global_ai_data["news"][:100]}...</span></div>', unsafe_allow_html=True)

# ================= ⚡ SEQUENTIAL SCANNER (RAM GUARDIAN) =================
def fetch_coin_radar(coin):
    try:
        # লিমিট ১৫০ রাখা হয়েছে যাতে RAM 512MB ক্রস না করে
        ...
        if df['ema_9'].iloc[-1] > df['ema_20'].iloc[-1]: reasons.append("✅ EMA 9 crossed above EMA 20 (Bullish)")
        else: reasons.append("❌ EMA 9 is below EMA 20 (Bearish)")
        
        if curr_vol > vol_sma: reasons.append(f"✅ Volume is High ({vol_ratio:.0f}% of average)")
        else: reasons.append(f"❌ Volume is Low ({vol_ratio:.0f}% of average)")
        
        reasons.append(f"📊 RSI Level: {rsi:.1f}")
        ...
        if curr_price > df['ema_50'].iloc[-1] and df['ema_9'].iloc[-1] > df['ema_20'].iloc[-1] and curr_vol > vol_sma and rsi < 65:
            ...
            is_signal, signal_dir, alloc, signal_text = True, "SHORT", 5.0, "🧨 SNIPER SELL SIGNAL MATCHED!"
...
# 🧠 Sequential Scanning (একের পর এক স্ক্যান করে র‍্যাম বাঁচাবে)
radars = {}
for coin in SCALPING_COINS:
    res = fetch_coin_radar(coin)
    if res: radars[coin] = res
...
for symbol, pos in st.session_state.bot_positions.items():
    if symbol in radars and radars[symbol]:
        ...
        if (pos['dir'] == "LONG" and current_price >= pos['tp']) or (pos['dir'] == "SHORT" and current_price <= pos['tp']): close_trade, reason = True, "🎯 Auto Target Hit"
        elif (pos['dir'] == "LONG" and current_price <= pos['sl']) or (pos['dir'] == "SHORT" and current_price >= pos['sl']): close_trade, reason = True, "🛑 Stop Loss Hit"
        elif is_critical_danger: close_trade, reason = True, "🚨 AI Emergency Exit" # গ্লোবাল এআই চেক
...
if st.session_state.auto_trade_active and not is_critical_danger: # ডেঞ্জার থাকলে নতুন ট্রেড নেবে না
    for symbol, data in radars.items():
        ...
        if symbol not in st.session_state.bot_positions and data['is_signal'] and bot_is_warmed_up and symbol not in st.session_state.cooldowns:
            ...
            if st.session_state.available_balance_inr >= invest_amount and invest_amount > 10:
                ...
                st.toast(f"✅ Executed {data['signal']} on {symbol} (Margin: ₹{invest_amount:.2f})", icon="🚀")
                data_changed = True
...
st.markdown(f"""
    <div style="display:flex; justify-content:space-between; text-align:center; background: #14181C; border: 1px solid #2B3139; border-radius: 8px; padding: 10px; margin-bottom: 15px;">
        ...
        <div style="flex: 1; border-left: 1px solid #2B3139;"><div style="font-size:10px; color:#848E9C;">MARGIN IN USE</div><div style="font-size:14px; font-weight:bold; color:#FCD535;">₹{active_invested:,.1f}</div></div>
    </div>
""", unsafe_allow_html=True)
...
if st.session_state.active_coin in radars and radars[st.session_state.active_coin]:
    ...
    with col1: st.markdown(f"<div class='analyzer-card'><div class='analyzer-title'>🧭 MARKET</div><div class='analyzer-val' style='color:{data['t_color']}'>{data['trend']}</div></div>", unsafe_allow_html=True)
...
# ================= 💼 LIVE POSITIONS WITH CLEAR INVESTMENT DETAILS =================
if st.session_state.bot_positions:
    st.markdown("<hr><div style='color:#848E9C; font-size:12px; font-weight:bold; margin-bottom:10px;'>💼 ACTIVE POSITIONS</div>", unsafe_allow_html=True)
    for c, p in list(st.session_state.bot_positions.items()):
        border_color = "#00FF00" if p['dir'] == "LONG" else "#FF1744"
        total_leveraged_size = p['invested_inr'] * VIRTUAL_LEVERAGE
        
        st.markdown(f"""
            <div class="pos-card" style="border-left-color: {border_color};">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    <div>
                        <div><span style="font-weight:bold; font-size:16px;">{c}</span> <span style="font-size:11px; padding:2px 4px; border-radius:4px; background:rgba(255,255,255,0.1); color:{border_color};">{p['dir']} (10x Leverage)</span></div>
                        <div style="font-size:11px; color:#848E9C; margin-top:6px;">
                            🎯 <b>Your Bet/Margin:</b> ₹{p['invested_inr']:,.2f}<br>
                            🚀 <b>Total Position Size (10x):</b> ₹{total_leveraged_size:,.2f}<br>
                            📍 <b>Entry:</b> {p['entry']} | <b>SL:</b> {p['sl']:.4f} | <b>TP:</b> {p['tp']:.4f}
                        </div>
                    </div>
                    <div style="text-align:right;">
                        <div style="font-size:10px; color:#848E9C;">LIVE PNL</div>
                        <div style="font-weight:bold; font-size:18px; color:{'#00FF00' if p['live_pnl'] >= 0 else '#FF1744'};">₹{p['live_pnl']:,.2f}</div>
                    </div>
                </div>
            </div>
        """, unsafe_allow_html=True)
        # ভূতুড়ে বাটন ফিক্সড - সাথে সাথে কাজ করবে
        if st.button(f"💰 Close {c} Manually", key=f"close_{c}"):
            fee_inr = total_leveraged_size * FEE_RATE * 2
...
# লুপ রিফ্রেশ (ভূতুড়ে বাটন কমানোর জন্য স্লিপ টাইম কমানো হয়েছে)
time.sleep(2)
st.rerun()
