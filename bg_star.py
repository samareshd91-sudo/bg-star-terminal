...
st.set_page_config(page_title="BG STAR PRO", layout="wide", initial_sidebar_state="collapsed")
...
st.markdown("""
    ...
    .alert-box { background-color: #FF1744; color: white; padding: 10px; text-align: center; font-weight: bold; font-size: 14px; border-radius: 8px; margin-bottom: 15px; animation: blinker 1.5s linear infinite; }
...
def get_real_ai_sentiment(coin_symbol):
    try:
        ...
        if not latest_news:
            return {"label": "neutral", "news": f"No immediate news for {coin_symbol}."}
        ...
        return {"label": "neutral", "news": "AI Offline. Relying strictly on Technicals."}
...
def fetch_coin_radar(coin):
    try:
        ...
        # Local Support/Resistance & ATR for Risk Management
        ...
        # Strategy Logic (Trend + Volume)
        if curr_price > df['ema_50'].iloc[-1]: reasons.append("✅ Price Above EMA 50")
        else: reasons.append("❌ Price Below EMA 50")
        
        if df['ema_9'].iloc[-1] > df['ema_20'].iloc[-1]: reasons.append("✅ EMA 9 > 20 (Bullish Momentum)")
        else: reasons.append("❌ EMA 9 < 20 (Bearish Momentum)")
        
        if curr_vol > vol_sma: reasons.append("✅ Volume Spike Detected")
        ...
        return {
            ...
            'reasons': reasons, 'is_signal': is_signal, 'alloc': alloc, 
...
# Safely close positions (Loop Crash Fix)
positions_to_close = []
for symbol, pos in st.session_state.bot_positions.items():
    if symbol in radars and radars[symbol]:
        ...
        if (pos['dir'] == "LONG" and current_price >= pos['tp']) or (pos['dir'] == "SHORT" and current_price <= pos['tp']): 
            close_trade, reason = True, "🎯 Auto TP Hit"
        elif (pos['dir'] == "LONG" and current_price <= pos['sl']) or (pos['dir'] == "SHORT" and current_price >= pos['sl']): 
            close_trade, reason = True, "🛑 Auto SL Hit"
        elif is_critical_danger and symbol.split("/")[0] == active_sym: 
...
for symbol, reason in positions_to_close:
    ...
    # Real Fee Calculation on Leveraged Amount
    ...
    st.session_state.cooldowns[symbol] = time.time() + 300 # 5 min cooldown
...
if st.session_state.auto_trade_active:
    for symbol, data in radars.items():
        ...
        # Block if AI says Danger for this specific active coin
        ...
        if symbol not in st.session_state.bot_positions and data['is_signal'] and bot_is_warmed_up and symbol not in st.session_state.cooldowns:
            ...
            if st.session_state.available_balance_inr >= invest_amount and invest_amount > 10:
                ...
                if data['signal'] == "LONG":
                    ...
                    tp = data['price'] + ((data['price'] - sl) * 1.5) 
                else:
                    sl = data['res'] + atr_buffer 
                    tp = data['price'] - ((sl - data['price']) * 1.5)
...
st.markdown(f"""
    <div style="background: linear-gradient(135deg, #14181C 0%, #0B0E11 100%); border: 2px solid #2B3139; border-radius: 12px; padding: 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.5); margin-bottom: 10px;">
        <div style="display:flex; justify-content:space-between; text-align:center;">
            ...
            <div style="flex: 1; border-left: 1px solid #2B3139;"><div style="font-size:9px; color:#848E9C; font-weight:bold;">FEES (10x)</div><div style="font-size:13px; font-weight:bold; color:#FF1744;">-₹{st.session_state.total_fees_paid:,.1f}</div></div>
        </div>
    </div>
""", unsafe_allow_html=True)

# Chart & Transparent Logic
if st.session_state.active_coin in radars and radars[st.session_state.active_coin]:
    ...
    with col2:
        ...
        fig.update_layout(height=280, margin=dict(l=0,r=0,t=10,b=0), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis=dict(showgrid=False, rangeslider=dict(visible=False)), yaxis=dict(showgrid=True, gridcolor='#2B3139', side='right'))
...
if st.session_state.bot_positions:
    ...
    for c, p in list(st.session_state.bot_positions.items()):
        ...
        st.markdown(f"""
            <div class="pos-card" style="border-left-color: {border_color};">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    <div>
                        <div><span style="font-weight:bold; font-size:16px;">{c}</span> <span style="font-size:11px; padding:2px 4px; border-radius:4px; background:rgba(255,255,255,0.1); color:{border_color};">{p['dir']} (10x)</span></div>
                        <div style="font-size:11px; color:#848E9C; margin-top:4px;">Margin: ₹{p['invested_inr']:,.2f} | Entry: {p['entry']}</div>
                    </div>
        ...
        """, unsafe_allow_html=True)
        if st.button(f"💰 Close {c} Trade Manually", key=f"close_{c}"):
...
if st.session_state.trade_history:
    st.markdown("<hr><div style='color:#848E9C; font-size:11px; font-weight:bold; margin-bottom:10px;'>📜 RECENT TRADE LOGS</div>", unsafe_allow_html=True)
    for h in st.session_state.trade_history[:8]:
        h_color = "#00FF00" if h['pnl'] > 0 else "#FF1744"
        st.markdown(f"<div class='hist-row'><span><span style='color:#848E9C;'>{h['time']}</span> | <b>{h['coin']}</b> | {h['reason']}</span><span style='color:{h_color}; font-weight:bold;'>₹{h['pnl']:.2f}</span></div>", unsafe_allow_html=True)
